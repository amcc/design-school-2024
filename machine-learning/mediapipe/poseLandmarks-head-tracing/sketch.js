...
// canvas to trace heads
let headCanvas;

function setup() {
  createCanvas(windowWidth, windowHeight);
  headCanvas = createGraphics(windowWidth, windowHeight);
  captureWebcam();
}
...
function draw() {
  ...
  if (mediaPipe.landmarks.length > 0) {
    ...
    mediaPipe.landmarks.forEach((person, personIndex) => {
      ...
      // draw a circle on the nose in the headCanvas
      headCanvas.fill(0, 20);
      headCanvas.noStroke();
      let noseX = lerpLandmarks[personIndex][0].x;
      let noseY = lerpLandmarks[personIndex][0].y;
      let noseZ = lerpLandmarks[personIndex][0].z;

      // eye positions
      let leftEyeX = lerpLandmarks[personIndex][5].x;
      let leftEyeY = lerpLandmarks[personIndex][5].y;
      let rightEyeX = lerpLandmarks[personIndex][2].x;
      let rightEyeY = lerpLandmarks[personIndex][2].y;

      let eyeDistance = dist(leftEyeX, leftEyeY, rightEyeX, rightEyeY);
      if (frameCount > 100) headCanvas.circle(noseX, noseY, eyeDistance / 2);
    });
  }
